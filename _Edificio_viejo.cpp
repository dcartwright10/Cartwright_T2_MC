...
//generacion de 100 valores de frecuencia diferentes	
	double *omegas;
	omegas = new double[n];
 	omegas[0] = 0.2*sqrt(k/m);
	omegas[1] = 0.228*sqrt(k/m);
	double dif = omegas[1]-omegas[0];
	double *maximos;
	maximos = new double[n];
	double max= -100;

	//for(int k=2; k<=n; k++){
		//omegas[k] = omegas[k-1]+dif;
